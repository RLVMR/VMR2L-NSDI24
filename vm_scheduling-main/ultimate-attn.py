...
def parse_args():
    ...
    parser.add_argument("--accum-iter", type=int, default=6,
...
class Agent(nn.Module):
    ...
    def get_value(self, obs_info_pm, obs_info_all_vm, obs_info_edges, obs_info_num_steps, obs_info_num_vms):
        ...
        if self.model == "attn":
            return self.vm_net(obs_info_all_vm, obs_info_num_steps, obs_info_pm, obs_info_edges, num_vms_mask)[3]
        else:
    ...
    def get_action_and_value(self, envs, obs_info_pm, obs_info_all_vm, obs_info_edges, obs_info_num_steps,
        ...
        if self.model == "attn":
            pm_embed, vm_embed, vm_logits, critic_score, attn = self.vm_net(obs_info_all_vm, obs_info_num_steps,
        ...
        pm_attn = attn[-1][torch.arange(b_sz, device=self.device), selected_vm][:, 1:1 + self.num_pm]
        ...
        if self.model == "attn":
            pm_logits = self.pm_net(vm_embed[torch.arange(b_sz), selected_vm].unsqueeze(1),
                                    ...
                                    obs_info_pm, pm_embed, pm_attn)  # b_sz
        else:
            raise ValueError(f'self.model={self.model} is not implemented')

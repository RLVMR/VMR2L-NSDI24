...
def parse_args():
    ...
    parser.add_argument("--gym-id", type=str, default="graph-v1",
...
class Agent(nn.Module):
    def __init__(self, gcn_net, vm_net, pm_net, params, args_model):
    ...
    def get_value(self, obs_info_pm, obs_info_all_vm, obs_info_num_steps, obs_info_num_vms, graphs):
        num_vms_mask = torch.arange(self.num_vm, device=obs_info_num_vms.device)[None, :] >= obs_info_num_vms[:, None]
        obs_pm_embed, obs_vm_embed = self.gcn_net(graphs, graphs.ndata['x'], obs_info_num_steps.shape[0])
        obs_vm_embed[num_vms_mask] = 0
        obs_vm_embed = torch.cat([obs_vm_embed, obs_info_all_vm], dim=-1)
        obs_pm_embed = torch.cat([obs_info_pm, obs_pm_embed], dim=-1)
        return self.vm_net(obs_vm_embed, obs_info_num_steps, obs_pm_embed, num_vms_mask)[1]

    def get_action_and_value(self, envs, obs_info_pm, obs_info_all_vm, obs_info_num_steps, obs_info_num_vms,
                             graphs, pm_mask=None, selected_vm=None, selected_pm=None):
        if pm_mask is None:
        ...
        b_sz = obs_info_num_steps.shape[0]
        # obs_info_all_vm: torch.Size([8, 2089, 14])
        # obs_info_pm:  torch.Size([8, 279, 8])

        obs_pm_embed, obs_vm_embed = self.gcn_net(graphs, graphs.ndata['x'], b_sz)
        obs_vm_embed[num_vms_mask] = 0
        obs_vm_embed = torch.cat([obs_vm_embed, obs_info_all_vm], dim=-1)
        obs_pm_embed = torch.cat([obs_info_pm, obs_pm_embed], dim=-1)
        vm_logits, critic_score = self.vm_net(obs_vm_embed, obs_info_num_steps, obs_pm_embed, num_vms_mask)
        ...
        # obs_info_all_vm:  torch.Size([8, 2089, 14])
        pm_logits = self.pm_net(obs_vm_embed[torch.arange(b_sz), selected_vm].unsqueeze(1).detach(),
                                obs_info_num_steps, obs_pm_embed.detach())  # pm_logits:  torch.Size([8, 279])
...
def make_batch_graph(num_envs, edges, obs_pm, obs_vm, n_pms, node_class, num_vms):
    graphs = []
    for i in range(num_envs):
        g = dgl.graph((edges[i, :num_vms[i], 0], edges[i, :num_vms[i], 1]),
                      num_nodes=n_pms + 2089, idtype=torch.int32)
        g.ndata['x'] = torch.cat([torch.cat([obs_pm[i], obs_vm[i]], dim=0),
                                  node_class], dim=-1)
...
if __name__ == "__main__":
    ...
    gcn_model = models.GCN_Wrapper(params).model
    # input the vm candidate model
    if args.model == 'attn':
        vm_cand_model = models.VM_Attn_Graph_Wrapper(params, args.pretrain).model
        pm_cand_model = models.PM_Attn_Graph_Wrapper(params).model
    else:
        raise ValueError(f'args.model = {args.model} is not defined or not supported!')

    agent = Agent(gcn_model, vm_cand_model, pm_cand_model, params, args.model)
    ...
    obs_vm = torch.zeros(num_steps, num_envs, params.num_vm, params.vm_cov, device=device)
    obs_pm = torch.zeros(num_steps, num_envs, params.num_pm, params.pm_cov, device=device)
    obs_num_steps = torch.zeros(num_steps, num_envs, 1, 1, device=device)
    obs_num_vms = torch.zeros(num_steps, num_envs, dtype=torch.int32, device=device)
    obs_edges = torch.zeros(num_steps, num_envs, params.num_vm, 2, dtype=torch.int32, device=device)
    vm_actions = torch.zeros(num_steps, num_envs, device=device)
    pm_actions = torch.zeros(num_steps, num_envs, device=device)
    ...
    action_masks = torch.zeros(num_steps, num_envs, envs.single_action_space.nvec[1], dtype=torch.bool,
                               device=device)
    ...
    node_class = torch.tensor(np.concatenate([np.zeros((params.num_pm, 1)),
                                              np.zeros((params.num_vm, 1))], axis=0), dtype=torch.float32,
                              device=device)
    ...
    for update in pbar:
        ...
        current_ep_info = np.zeros((num_steps, num_envs, 2)) - 1000  # return, len, fr
        ...
        for step in range(0, num_steps):
            ...
            graphs = make_batch_graph(num_envs, next_obs_edges, next_obs_pm, next_obs_vm, n_pms,
                                      node_class, next_obs_num_vms)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs_pm, next_obs_vm, next_obs_num_steps,
                                         next_obs_num_vms, graphs).reshape(1, -1)
            if args.gae:
        ...
        b_obs_edges = obs_edges.reshape(-1, params.num_vm, 2)
        ...
        for epoch in range(args.update_epochs):
            ...
            for index, start in enumerate(range(0, args.batch_size, args.minibatch_size)):
                ...
                b_graphs = make_batch_graph(args.minibatch_size, b_obs_edges[mb_inds], b_obs_pm[mb_inds],
                                            b_obs_vm[mb_inds], n_pms, node_class.detach(),

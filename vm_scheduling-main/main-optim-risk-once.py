...
def parse_args():
    ...
    parser.add_argument("--num-envs", type=int, default=8,
                        help="the number of parallel game environments")
    parser.add_argument("--num_collect_steps", type=int, default=5,
                        help="the number of total environments for training")
    ...
    num_train_envs = args.num_envs * args.num_collect_steps
    args.batch_size = int(num_train_envs * args.num_steps)
...
if __name__ == "__main__":
    ...
    num_train_envs = num_envs * num_collect_steps
    num_trials = args.num_trials
    less_feature = args.less_feature
    num_test_steps = 8 * args.max_steps
    eff_b_sz = args.accum_iter * args.minibatch_size
    run_name = f"tm19_g0_{args.vm_data_size}{args.max_steps}_{args.gym_id}_{args.model}_{args.seed}" \
    ...
    if args.model == 'attn':
        ...
        pm_cand_model = models.PM_Attn_Flip_Wrapper(params).model
    elif args.model == 'mlp':
    ...
    obs_vm = torch.zeros(num_steps, num_trials, num_train_envs, params.num_vm, params.vm_cov, device=device)
    obs_pm = torch.zeros(num_steps, num_trials, num_train_envs, params.num_pm, params.pm_cov, device=device)
    obs_num_steps = torch.zeros(num_steps, num_trials, num_train_envs, 1, 1, device=device)
    obs_num_vms = torch.zeros(num_steps, num_trials, num_train_envs, dtype=torch.int32, device=device)
    vm_actions = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    pm_actions = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    logprobs = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    rewards = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    dones = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    values = torch.zeros(num_steps, num_trials, num_train_envs, device=device)
    # envs.single_action_space.nvec: [2089, 279] (#vm, #pm)
    action_masks = torch.zeros(num_steps, num_trials, num_train_envs, envs.single_action_space.nvec[1],
                               dtype=torch.bool,
                               device=device)
    ...
    for update in pbar:
        ...
        current_ep_info = np.zeros((num_steps, num_trials, num_train_envs, 2)) - 1000  # return, len, fr
        current_ep_final_fr = torch.zeros(num_trials, num_train_envs, device=device)  # return, len, fr

        for env_start in range(0, num_train_envs, num_envs):
            env_end = env_start + num_envs
            ...
            for trial in range(num_trials):
                ...
                if less_feature:
                    next_obs_vm = torch.cat([next_obs_vm[:, :, :10],
                                             next_obs_vm[:, :, 11].unsqueeze(-1),
                                             next_obs_vm[:, :, 13].unsqueeze(-1)], dim=-1)
                ...
                for step in range(num_steps):
                    ...
                    obs_pm[step, trial, env_start:env_end] = next_obs_pm
                    obs_vm[step, trial, env_start:env_end] = next_obs_vm
                    obs_num_steps[step, trial, env_start:env_end] = next_obs_num_steps
                    obs_num_vms[step, trial, env_start:env_end] = next_obs_num_vms
                    dones[step, trial, env_start:env_end] = next_done

                    with torch.no_grad():
                        vm_action, pm_action, logprob, _, value, action_mask \
                            = agent.get_action_and_value(envs, next_obs_pm, next_obs_vm,
                                                         next_obs_num_steps, next_obs_num_vms)
                        values[step, trial, env_start:env_end] = value.flatten()  # value:  torch.Size([8, 1])
                    action_masks[step, trial, env_start:env_end] = action_mask
                    vm_actions[step, trial, env_start:env_end] = vm_action
                    pm_actions[step, trial, env_start:env_end] = pm_action
                    logprobs[step, trial, env_start:env_end] = logprob
                    ...
                    next_obs_num_steps = torch.Tensor(next_obs_dict['num_steps']).to(device)
                    next_obs_num_vms = torch.tensor(next_obs_dict['num_vms'], dtype=torch.int32, device=device)
                    rewards[step, trial, env_start:env_end] = torch.tensor(reward, device=device).view(-1)
                    next_done = torch.Tensor(done).to(device)

                for env_id, item in enumerate(info):
                    current_ep_info[step, trial, env_start + env_id, 0] = item["episode"]["r"]
                    current_ep_info[step, trial, env_start + env_id, 1] = item['fragment_rate']
                    current_ep_final_fr[trial, env_start + env_id] = item['fragment_rate']
        ...
        with torch.no_grad():
            next_value = torch.zeros(1, num_train_envs, device=device)
            if args.gae:
        ...
        keep_index = torch.topk(current_ep_final_fr, 1, dim=0, largest=False)[1]
        keep_mask = torch.zeros_like(current_ep_final_fr, dtype=torch.bool)
        keep_mask[keep_index, torch.arange(num_train_envs).reshape(1, -1)] = True
        keep_mask = keep_mask.reshape(-1).repeat(num_steps)
        b_obs_vm = obs_vm.reshape(-1, params.num_vm, params.vm_cov)[keep_mask]
        b_obs_pm = obs_pm.reshape(-1, params.num_pm, params.pm_cov)[keep_mask]
        b_obs_num_steps = obs_num_steps.reshape(-1, 1, 1)[keep_mask]
        b_obs_num_vms = obs_num_vms.reshape(-1)[keep_mask]
        b_vm_actions = vm_actions.reshape(-1)[keep_mask]
        b_pm_actions = pm_actions.reshape(-1)[keep_mask]
        b_logprobs = logprobs.reshape(-1)[keep_mask]
        ...
        b_advantages = advantages.reshape(-1)[keep_mask]
        b_returns = returns.reshape(-1)[keep_mask]
        b_values = values.reshape(-1)[keep_mask]
        b_action_masks = action_masks.reshape(-1, envs.single_action_space.nvec[1])[keep_mask]
        ...
        assert args.batch_size == current_bsz

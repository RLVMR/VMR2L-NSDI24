...
def data_split(params):
    ...
    target_raw = np.float32(df.iloc[:, -1].values)
    target = np.zeros_like(target_raw)
    for i in range(len(params.quantiles) - 1):
        target[(params.quantiles[i] <= target_raw) & (target_raw <= params.quantiles[i+1])] = i
    print('target_raw: ', target_raw[:20])
    print('target_cat: ', target[:20])
    target = np.int64(target)
    vm_states = np.expand_dims(np.float32(df.iloc[:, -1-params.vm_cov:-1].values), axis=1)

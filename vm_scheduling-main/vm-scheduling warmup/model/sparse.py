...
class VM_candidate_model(nn.Module):
    ...
    def do_train(self, vm_states, pm_states, vm_pm_relation, labels):
        """ Train for one batch.
        Args:
        ...
        predict = self(vm_states, num_steps, pm_states, vm_pm_relation)
        loss = self.loss_fn(predict, labels)
        loss.backward()
        return [loss.item()], loss

    def test(self, vm_states, pm_states, vm_pm_relation, labels):
        """ Test for one batch.
        Args:
            vm_states ([batch_size, num_vm, vm_cov]): features of virtual machines
            pm_states ([batch_size, num_pm, pm_cov]): features of physical machines
        ...
        num_steps = torch.zeros(b_sz, 1, 1, device=vm_states.device)
        if self.output_categorical:
            logits, attn = self(vm_states, num_steps, pm_states, vm_pm_relation)
            predict = torch.argmax(logits, dim=-1, keepdim=True)
        else:
            predict, attn = self(vm_states, num_steps, pm_states, vm_pm_relation)
        return {
            'predictions': predict
        }

...
class AsyncVectorEnv_Patch(VectorEnv):
    ...
    def call_parse(self, name, *args, **kwargs):
        self.call_async_parse(name, *args, **kwargs)
        return self.call_wait()

    def call_async_parse(self, name, *args, **kwargs):
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
        ...
        kwargs_list = [dict(zip(kwargs, t)) for t in zip(*kwargs.values())]

        for pipe, kwarg in zip(self.parent_pipes, kwargs_list):
            pipe.send(("_call", (name, args, kwarg)))

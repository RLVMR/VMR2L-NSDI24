...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.pe = torch.zeros(self.seq_length, params.d_hidden, device=self.device)
        position = torch.arange(0, self.seq_length, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, params.d_hidden, 2).float() * (-math.log(10000.0) / params.d_hidden))
    ...
    def forward(self, vm_states, pm_states, return_attns=False):
        ...
        transformer_output = self.transformer(src=pm_encode + self.pe.detach(), tgt=vm_encode)

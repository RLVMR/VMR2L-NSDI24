...
def data_split(params):
    ...
    target = np.float32(df.iloc[:, -params.num_vm:].values) * 2
    vm_states = np.float32(df.iloc[:, -params.num_vm * (2 + params.vm_cov):-params.num_vm].values).reshape(-1, params.num_vm, params.vm_cov + 1)
